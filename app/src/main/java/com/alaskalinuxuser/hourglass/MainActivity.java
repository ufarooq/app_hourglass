...
public class MainActivity extends AppCompatActivity {
  ...
  protected void onSaveInstanceState(@NonNull Bundle outState) {
    ...
    Gson gson = new Gson();
    // Serialization
    outState.putString("timeCount", gson.toJson(timeCount));
    outState.putString("maxTime", gson.toJson(maxTime));
    outState.putString("curTime", gson.toJson(curTime));
    outState.putString("soundNum", gson.toJson(soundNum));
    outState.putString("recureTime", gson.toJson(recureTime));
    outState.putString("vibrateYes", gson.toJson(vibrateYes));

    outState.putString("recureYes", gson.toJson(recureYes));
    outState.putString("alreYes", gson.toJson(alreYes));
    outState.putString("manualTime", gson.toJson(manualTime));
    outState.putString("stoppedTimer", gson.toJson(stoppedTimer));
    outState.putString("recureTime", gson.toJson(recureTime));
    outState.putString("vibrateYes", gson.toJson(vibrateYes));
  ...
  protected void onRestoreInstanceState(@NonNull Bundle savedInstanceState) {
    ...
    Gson gson = new Gson();
    // Deserialization
    maxTime = gson.fromJson(savedInstanceState.getString("maxTime"), int.class);
    curTime = gson.fromJson(savedInstanceState.getString("curTime"), int.class);
    soundNum = gson.fromJson(savedInstanceState.getString("soundNum"), int.class);
    recureTime = gson.fromJson(savedInstanceState.getString("recureTime"), int.class);
    vibrateYes = gson.fromJson(savedInstanceState.getString("vibrateYes"), boolean.class);
    recureYes = gson.fromJson(savedInstanceState.getString("recureYes"), boolean.class);
    alreYes = gson.fromJson(savedInstanceState.getString("alreYes"), boolean.class);
    manualTime = gson.fromJson(savedInstanceState.getString("manualTime"), String.class);
    stoppedTimer = gson.fromJson(savedInstanceState.getString("stoppedTimer"), boolean.class);
    recureTime = gson.fromJson(savedInstanceState.getString("recureTime"), int.class);
    vibrateYes = gson.fromJson(savedInstanceState.getString("vibrateYes"), boolean.class);
    long end = Calendar.getInstance().getTimeInMillis();
    Log.i(
        SAVE_RESTORE_TAG,
        "Restore: " + getBundleSizeInBytes(savedInstanceState) + ":" + (end - start));
  }
  ...
  public void startCount(int allotedTime, final int countBy) {
    ...
    PendingIntent detailsPendingIntent =
        PendingIntent.getActivity(
            MainActivity.this, 0, detailsIntent, PendingIntent.FLAG_UPDATE_CURRENT);

    NotificationCompat.Builder builder =
        new NotificationCompat.Builder(context)
            .setSmallIcon(R.drawable.hourglass)
            .setWhen(System.currentTimeMillis() + recureTime)
            .setUsesChronometer(true)
            .setContentTitle("Hourglass")
            .setContentText(intentPhrase)
            .addAction(
    ...
    timeCount =
        new CountDownTimer(allotedTime, countBy) {
          ...
          public void onFinish() {
            ...
            if (recureYes) {
              ...
              Toast timeUp =
                  Toast.makeText(
                      getApplicationContext(), "Time is up! Restarting timer.", Toast.LENGTH_LONG);
              timeUp.show();
